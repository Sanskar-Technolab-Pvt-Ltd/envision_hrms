...
def execute(filters=None):
	...
	# Specify which earning and deduction components to include
	selected_earning_components = [
		"Basic Site", "Bonus Site", "Food Allowance Site", "HRA", "Leave Site", "Other Allowance Site", "Overtime Site", "Public Holiday"  # <-- Edit this list as needed
	]
	selected_deduction_components = [
		"ESI Employer", "PF Employer Site", "Professional Tax Site", "Provident Fund Site"  # <-- Edit this list as needed
	]

	earning_types = [e for e in earning_types if e in selected_earning_components]
	ded_types = [d for d in ded_types if d in selected_deduction_components]
	...
	for ss in salary_slips:
		...
		for e in earning_types:
			row.update({frappe.scrub(e): ss_earning_map.get(ss.name, {}).get(e, 0)})

		for d in ded_types:
			row.update({frappe.scrub(d): ss_ded_map.get(ss.name, {}).get(d, 0)})
